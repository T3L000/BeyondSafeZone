	...
	10: [
		{
			...
			"location": "school",
			"zombie_delta": -2,
			"public_clue": "去往学校方向的河道桥上，尸群在一夜之间稀疏了大半。桥面散落着未燃尽的照明棒和轮胎刹车痕。",
			"truth": "祁眠骑摩托在深夜抵达桥头，利用喇叭和燃烧物把尸群缓慢引离桥面，恢复了一条关键通行路线。",
			"ai_replay": "输入：桥梁被尸群占据、阻断幸存者转移路线、夜间视野允许隐蔽操作。规则：不能正面清怪，用噪音和光源缓慢引导，避免目击。",
			"subjective_fragment": "桥通了一条路。他们不知道是谁做的，这样最好。"
		}
	],
	11: [
		{
	...
	14: [
		{
			...
			"public_clue": "诊所门口多了一支未拆封的抗生素，上面还贴着撕了一半的旧处方。",
			"truth": "祁眠在红潮夜前最后巡查近圈，确认返生计划巡逻队今夜不会进入林行所在街区，留下药品应急。",
			"ai_replay": "输入：尸群迁移路线、巡逻队时间、近圈幸存者活动。规则：避开暴露，最后确认安全窗口。",
			"subjective_fragment": "趁他们还缩在营地里，把这里最后一扇窗也关好。"
		}
	],
	15: [
		{
		...
		{
			"title": "双层揭示",
			"public_clue": "筛查棚外有人低声议论：昨晚那股尸群像被人牵走了，还有人看到一辆摩托穿过了东线的封锁。",
			"truth": "林行只听见「尸群被牵走」「摩托穿东线」的异常线索，玩家随后看到祁眠完整行动回放。",
			"ai_replay": "输入：林行不可见；系统结算共享世界影响。规则：玩家获得上帝视角，角色仍保持误认。",
			"subjective_fragment": "如果那个在筛查棚排队的人是他，我也已经错过。但我不能停下来确认。"
		}
	]
}
