...
namespace BeyondSafeZone.Core
{
    ...
    public static class TextRenderer
    {
        ...
        /// <summary>结尾日志：祁眠人格卡、输入、行动排序和共享地图影响</summary>
        public static string GetQimianEndingRevealText(GameState state)
        {
            var lines = new List<string>();
            var card = state.Qimian.PersonalityCard;
            bool hasClinicHelpMark = state.PlayerMarks.TryGetValue("clinic", out var clinicMark) && clinicMark.Type == "help";
            bool hasClinicResponse = state.Qimian.Log.Any(log => log.Title == "响应玩家标记：社区诊所");
            bool hasClinicDossier = state.AnomalyDossier.Any(entry =>
                entry.LocationId == "clinic" &&
                entry.ClueText.Contains("匿名药品"));

            lines.Add("");
            lines.Add("═══ 祁眠结尾日志：隐藏行动解释 ═══");
            lines.Add("人格卡：");
            lines.Add($"  主要目标：{card.MainGoal}");
            lines.Add($"  暴露规则：{card.Exposure}");
            lines.Add($"  道德规则：{card.MoralRule}");
            lines.Add($"  资源规则：{card.ResourceRule}");
            lines.Add($"  保护区态度：{card.SafezoneAttitude}");
            lines.Add("");

            if (hasClinicHelpMark)
            {
                lines.Add("感知输入：");
                lines.Add($"  第{clinicMark.Day}天，社区诊所存在林行留下的求助标记：「{clinicMark.Note}」");
                lines.Add("  地点痕迹：诊所门口、药柜状态、近圈尸群压力、可匿名放置药品的位置。");
            }
            else
            {
                lines.Add("感知输入：");
                lines.Add("  未读取到社区诊所求助标记；祁眠只依据地点痕迹、尸群热点和广播线索行动。");
            }

            lines.Add("");
            lines.Add("候选行动：");
            lines.Add("  追踪祁烬、夜间巡逻、匿名补给、清理路线、休整避开筛查。");
            lines.Add("排序理由：");
            if (hasClinicHelpMark)
                lines.Add("  寻找祁烬仍是长期最高目标；但社区诊所的求助标记属于近处、低暴露、可匿名回应的求助，因此匿名补给压过普通巡逻。");
            else
                lines.Add("  没有玩家标记输入时，祁眠按祁烬线索、暴露风险、尸群热点和资源需求排序。");

            lines.Add("最终选择：");
            if (hasClinicResponse)
            {
                var responseLog = state.Qimian.Log.First(log => log.Title == "响应玩家标记：社区诊所");
                lines.Add($"  第{responseLog.Day}天，{responseLog.Title}：匿名补给。");
                if (!string.IsNullOrEmpty(responseLog.SubjectiveFragment))
                    lines.Add($"  主观残句：{responseLog.SubjectiveFragment}");
            }
            else if (hasClinicHelpMark)
            {
                lines.Add("  祁眠读取过社区诊所求助标记，但本轮没有形成匿名药品回应。");
            }
            else
            {
                lines.Add("  本轮没有社区诊所求助标记链路。");
            }

            lines.Add("地图影响：");
            if (hasClinicResponse && state.Locations.TryGetValue("clinic", out var clinic))
            {
                int meds = clinic.Resources.TryGetValue("meds", out int value) ? value : 0;
                lines.Add($"  社区诊所药品余量变为 {meds}，地点出现祁眠痕迹，求助标记旁出现浅箭头。");
                lines.Add(hasClinicDossier
                    ? "  异常档案记录：匿名药品，结论为未知行动者能理解标记。"
                    : "  异常档案尚未记录匿名药品。");
            }
            else
            {
                lines.Add("  没有社区诊所匿名药品或浅箭头变化。");
            }
