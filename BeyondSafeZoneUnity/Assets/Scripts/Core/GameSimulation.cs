...
namespace BeyondSafeZone.Core
{
    ...
    public static class GameSimulation
    {
        ...
        /// <summary>获取祁眠结尾日志文本（结尾展示用）</summary>
        public static string GetQimianEndingRevealText(GameState state) =>
            TextRenderer.GetQimianEndingRevealText(state);
        ...
        {
            int day = state.Day;
            int publicClueCountBeforeNight = state.Qimian.PublicClues.Count;
            QimianController.ResolveForDay(state, day);
            string result = NightController.Resolve(state);
            var newPublicClues = state.Qimian.PublicClues
                .Skip(publicClueCountBeforeNight)
                .Where(clue => !string.IsNullOrEmpty(clue) && !result.Contains(clue))
                .ToList();
